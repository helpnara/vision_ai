    ...
    with st.expander("영상이 없다면 — 시험용 영상 만들기"):
        ...
        if st.button("🎬 시험용 영상 만들기", key="video_sample"):
            try:
                ...
                st.session_state[f"{VIDEO_PREFIX}_source"] = ui.SOURCE_LIST
...
# 1단계 영상 선택 위젯의 key 접두사. 공용 선택기(`ui.video_source`)에 넘긴다.
VIDEO_PREFIX = "video"


def _video_source() -> Path | None:
    """쓸 영상을 정한다. 4단계 판정 영상과 같은 선택기를 쓴다."""
    return ui.video_source(VIDEO_PREFIX)
