...
def evaluate(
    ...
    *,
    ...
    헛알람은 **정답 구간과 한 곳도 겹치지 않은** 예측 구간이다. 정답 구간을 조금 빗맞힌
    것까지 헛알람으로 세면, 잡았는데 벌점을 받는 셈이 된다.
    """
    if not (len(truth) == len(predicted) == len(seconds)):
        raise ValueError("정답·판정·시각의 길이가 서로 다릅니다.")

    truth_runs = runs(truth, seconds, gap=gap)
    predicted_runs = runs(predicted, seconds, gap=gap)
    ...
    false_alarms = [
        alarm for alarm in predicted_runs
        if not any(alarm.overlaps(segment) for segment in truth_runs)
    ]

    return SegmentReport(
        truth=tuple(truth_runs),
        predicted=tuple(predicted_runs),
        caught=tuple(caught),
        missed=tuple(missed),
